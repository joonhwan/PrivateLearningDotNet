...
namespace WcfCors
{
    ...
    public class CorsEnabledServiceHost : ServiceHost
    {
        ...
        {
            var endpoint = Description.Endpoints.FirstOrDefault(serviceEndpoint => serviceEndpoint.Binding is WebHttpBinding);
            if (endpoint == null)
            {
                endpoint = this.AddServiceEndpoint(this.contractType, new WebHttpBinding(), "");
            }
            ...
            if (endpoint.Behaviors.Find<CorsEnabledEndpointBehavior>() == null)
            {
                endpoint.Behaviors.Add(new CorsEnabledEndpointBehavior());
            }
        ...
        private void AddPreflightOperations(ServiceEndpoint endpoint, List<OperationDescription> corsOperations)
        {
            ...
            foreach (var operation in corsOperations)
            {
                ...
                else
                {
                    ...
                    {
                        var inputMessage = new MessageDescription(operation.Messages[0].Action + CorsConstants.PreflightSuffix, MessageDirection.Input);
                        ...
                        var outputMessage = new MessageDescription(operation.Messages[1].Action + CorsConstants.PreflightSuffix, MessageDirection.Output);
