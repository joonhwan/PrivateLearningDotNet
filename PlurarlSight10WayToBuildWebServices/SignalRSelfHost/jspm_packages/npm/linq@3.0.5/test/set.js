...
test("distinctUntilChanged", function() {
  ...
  [1, 3, 3, 3, 1, 2, 6, 3, 5, 1].select("{test:$}").distinctUntilChanged("$.test").is({test: 1}, {test: 3}, {test: 1}, {test: 2}, {test: 6}, {test: 3}, {test: 5}, {test: 1});
  [1].distinctUntilChanged().is(1);
  [1, 1].distinctUntilChanged().is(1);
  [1, 2].distinctUntilChanged().is(1, 2);
});
test("except", function() {
  actual = Enumerable.from([1, 3, 5, 6, 6, 3, 4, 3, 2, 9]).except([4, 6, 2, 7, 8, 10, 11]).toArray();
  deepEqual(actual, [1, 3, 5, 9]);
  actual = Enumerable.range(1, 10).select("{test:$%3}").except(Enumerable.range(1, 10).select("{test:$%2}"), "$.test").toArray();
  deepEqual(actual, [{test: 2}]);
});
test("intersect", function() {
  actual = Enumerable.from([1, 3, 5, 6, 6, 3, 4, 3, 2, 9]).intersect([4, 6, 2, 7, 8, 10, 11]).toArray();
  deepEqual(actual, [6, 4, 2]);
  actual = Enumerable.range(1, 10).select("{test:$%3}").intersect(Enumerable.range(1, 10).select("{test:$%2}"), "$.test").toArray();
  deepEqual(actual, [{test: 1}, {test: 0}]);
});
test("sequenceEqual", function() {
  ...
  ok(!Enumerable.range(1, 10).select("{test:$%3}").sequenceEqual(Enumerable.range(1, 10).select("{test:$%2}"), "$.test"));
  ok(Enumerable.range(1, 10).select("{test:$%3}").distinct("$.test").sequenceEqual([{test: 1}, {test: 2}, {test: 0}], "$.test"));
});
test("union", function() {
  actual = Enumerable.from([1, 3, 5, 6, 6, 3, 4, 3, 2, 9]).union([4, 6, 2, 7, 8, 10, 11]).toArray();
  deepEqual(actual, [1, 3, 5, 6, 4, 2, 9, 7, 8, 10, 11]);
  actual = Enumerable.range(1, 3).select("{test:$}").union(Enumerable.range(2, 3).select("{test:$}"), "$.test").toArray();
