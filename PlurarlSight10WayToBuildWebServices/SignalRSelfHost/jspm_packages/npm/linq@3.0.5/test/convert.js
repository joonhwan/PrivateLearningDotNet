...
test("toLookup", function() {
  ...
  actual = Enumerable.from(fileList).toLookup("file=>file.match(/\\.(.+$)/)[1]", "file=>file +'ele'", function(s) {
...
test("toJSONString", function() {
  ...
  actual = Enumerable.from(["a", "b", "c"]).toJSONString(function(key, value) {
    if (typeof value === 'object')
      return value;
    return value.toString().toUpperCase();
  });
  equal(actual, '["A","B","C"]');
  actual = Enumerable.from([1, 2, 3, 4, 5]).toJSONString(function(key, value) {
    return value;
  }, 1);
  ok(actual.indexOf("\n") != -1);
});
